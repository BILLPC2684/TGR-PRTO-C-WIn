...
#include  <stdlib.h>
#include  <direct.h>
DWORD CALLBACK CPU_CLOCK(void* null) {
  double dummy;
  while (true) {
    ...
    if (CPU.time % 1000 == 0) {
      ...
      if (showInfo == true) {
        ...
        printf("FPS: %d\t | IPS: %d\t | TotalRan: %p\t| CPU Run Quality: %.2lf%%\n", FPS, (double)IPS, TIPS, RunQuality);
        if (p == true) {
...
DWORD CALLBACK CPU_EXEC(void* null) {
  ...
  char*      extSAV  = (char*)malloc(1024); extSAV="";
  char*      execLoc = (char*)malloc(1024); execLoc=_getcwd(NULL, 0);
  char*      FN = (char*)malloc(1024); FN="";
  long       TIPS = 0;
  FILE*      SAV_file;
  char*      SN = (char*)malloc(1024); SN="";
  FileStruct SAV;
  LED[0] = 0xFF;
  LED[1] = 0x16;
  LED[2] = 0x16;
  ...
  char* BN = (char*)malloc(sizeof(char) * 1024); BN = _getcwd(NULL, 0);
  strcat_s(BN, sizeof(char) * 1024, "\\bin\\BIOS.tgr");
  ...
  if (ROM.found == false) { CPU.running = false; return 0; }
  else { CPU.running = true; }
  if (ROM.size / 1024 / 1024 >= 16) { printf("EMU ERROR: the ROM given was too big(%d MB/16 MB)", ROM.size / 1024 / 1024); return 0; }
  if (extSAV == "") {
    char* SN = (char*)malloc(sizeof(char) * (strlen(FN) + 1));
    crop(SN, FN, 0, strlen(FN) - 4);
    strcat_s(SN, sizeof(SN), ".sav");
    SAV = load(SN, 1);
  }
  else {
  ...
  if (ROM.data[0] == 1) {
    ...
    char* TN = (char*)malloc(sizeof(char) * 32);
    strcat_s(TN, sizeof(TN), "TheGameRazer - ");
    for (int i = 0; i < 8; i++) { strcat_s(TN, sizeof(TN), &ROM.data[i + 4]); } //Title_Name[i+16] = ROM.data[i+4]; }
  ...
  while (CPU.running) {
    ...
    if (CPU.reset == true) {
      ...
      SDL_Delay(1000);
      if (ROM.data[0] == 1) {
        char* TN = (char*)malloc(sizeof(char) * 8);
        strcat_s(TN, sizeof(TN), "TheGameRazer - ");
        for (int i = 0; i < 8; i++) { strcat_s(TN, sizeof(TN), ROM.data[i + 4]); }
        for (int i = 0; i < 23; i++) { Title_Name[i] = TN[i]; }
        Title_lock = false;
      } SDL_Delay(1000);
    }
    ...
    case 0x01:
      ...
      if (IMM == 0) {
        ...
        if ((CPU.REGs[A] + CPU.REGs[B]) > 0xFFFF) { CPU.flag[0] = 1; }
        else { CPU.flag[0] = 0; }
      }
      else {
        CPU.REGs[C] = (CPU.REGs[A] + (IMM % 0x10000)) % 0x10000;
        if ((CPU.REGs[A] + (IMM % 0x10000)) > 0xFFFF) { CPU.flag[0] = 1; }
        else { CPU.flag[0] = 0; }
      } CPU.flag[1] = 0;
      break;
    case 0x02:
      ...
      if (IMM == 0) {
        CPU.REGs[C] = (CPU.REGs[A] - CPU.REGs[B]) % 0x10000;
        if ((CPU.REGs[A] - CPU.REGs[B]) < 0) { CPU.flag[1] = 1; }
        else { CPU.flag[1] = 0; }
      }
      else {
        CPU.REGs[C] = (CPU.REGs[A] - (IMM % 0x10000)) % 0x10000;
        if ((CPU.REGs[A] - (IMM % 0x10000)) < 0) { CPU.flag[1] = 1; }
        else { CPU.flag[1] = 0; }
      } CPU.flag[0] = 0;
      break;
    case 0x03:
    ...
    case 0x0F:
      ...
      if (C == 0) {
        ...
        if (CPU.REGs[A] == IMM % 0x10000) { if (CPU.debug == true) { printf("True\n"); } }
        else { if (CPU.debug == true) { printf("False\n"); } CPU.IP = CPU.IP + 6; }
      } break;
    case 0x10:
    ...
    case 0x17:
      ...
      SAV_file = fopen_s(SN, sizeof(SN), "wb");
      rewind(SAV_file);
